...
class RoICapture:
    ...
    def get_regions(self):
        ...
        def get_skills_regions():
            ...
            line_width = wd*0.075
            left_shift = wd*0.009
            line_v_spacing = ht*0.017
            line_h_spacing = wd*0.248
            x1 = int(wd*0.178)
            y = int(ht*0.314)
    ...
    def get_regions_info(self):
        ...
        def get_lifebars_status():
            for x in range(6):
                ...
                # print(roi) # debug
                self.lifebars[x] = (any((n[0]==255 for n in roi)))
        ...
        def get_skills_status():
            ...
            for x in range(4):
                for y in range(3):
                    ...
                    self.skills_val[y, x] = any([sum(n)>735 for n in roi])
        ...
        def get_tiles_color():
            ...
            for x in range(8):
                for y in range(8):
                    ...
                    elif s > 60 and 20 < h < 30:
                        self.tiles_val[x,y] = 2
                    elif s > 60 and 45 < h < 55:
...
class Bot:
    ...
    def input(self, lifebars, skills_val, tiles_val, player_turn, plcmt):
        ...
        if plcmt == False and ((not any(enemy_life)) or (not any(player_life))):
            if self.get_lifebars_timer():
                self.game_over('win' if player_life else 'lost')
            else:
                pass

        elif np.any(skills_val == True):
            # extract coords from np.where with the tuple(zip(*...)) method
            skills = tuple(zip(*np.where(skills_val == True)))
            print("skills val :\n", self.skills, "skills ready for use:", skills)
            [self.skills_rdy.add(i) for i in skills]
        ...
        if player_turn:
            ...
            elif self.get_action_timer():               
                if self.skills_rdy:
                    mouse.click_skill(self.skills_rdy.pop())
                    print(self.skills_rdy)
                    self.last_action_time = time.time()
    ...
    def get_action_timer(self):
        if self.last_action_time:
            if (time.time() - self.last_action_time) > self.action_timer:
                self.last_action_time = time.time()
                return True
            else:
                return False
...
class MouseControl:
    ...
    def click_skill(self, coords):
        ...
        pyautogui.click()
        bx, by = self.radialskill[coords[1], coords[0]]
        # print("bx, by :", bx, by)
    ...
    def click_tile(self, tile):
        ...
        # tile_coordinates:
        tx = self.t0[0] + int(tile[1] * self.tile_spacing)
        ty = self.t0[1] + int(tile[0] * self.tile_spacing)
        
        pyautogui.moveTo(tx, ty, 0.25)
        pyautogui.click()
        pyautogui.moveTo(self.button[0], self.button[1], 0.25)
        pyautogui.click()


    def drag_tile(self, tiles):
        
        ax = self.t0[0] + tiles[0][1] * self.tile_spacing
        ay = self.t0[1] + tiles[0][0] * self.tile_spacing
        bx = self.t0[0] + tiles[1][1] * self.tile_spacing
        by = self.t0[1] + tiles[1][0] * self.tile_spacing
